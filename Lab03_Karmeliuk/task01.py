...
if __name__ == "__main__":
    ...
    plt.title("Метод вікна Парзена фіксованої ширини ")
    ...
    h_axes = plt.axes([0.20, 0.12, 0.40, 0.02], facecolor='blue')
    ...
    current_miss = ValueBuilder.classify_miss_by_best(graphic_builder.classified_dots,
    ...
    button_h_axes = plt.axes([0.05, 0.14, 0.05, 0.05])
    button_h = Button(button_h_axes, 'Навчити h')
    ...
    def learn_h(event):
        best_h = ValueBuilder.select_best_h(graphic_builder.classified_dots,
                                            graphic_builder.main_classified,
                                            graphic_builder.DISTANCE_ALGORITHM,
                                            graphic_builder.KERNEL_ALGORITHM)

        h_slider.set_val(best_h)


    def learn_kernel(event):
        fig.canvas.draw_idle()
        kernel_index = ValueBuilder.select_best_kernel(graphic_builder.classified_dots,
                                                       graphic_builder.main_classified,
                                                       graphic_builder.DISTANCE_ALGORITHM,
    ...
    def learn_best_percentage(event):
        fig.canvas.draw_idle()
        best_percentage = ValueBuilder.find_best_percentage(graphic_builder.main_classified,
                                                            graphic_builder.DISTANCE_ALGORITHM,
